...
app.get("/download", async (req, res) => {
  try {
    ...
    const title = file.title
      .replace(/[-&\/\\#, +()$~%.'":*?<>{}]/g, " ")
      .trim();
    const contentType = file.file_type === "video" ? "video/mp4" : "audio/mp3";
    ...
    const headers = {
      ...
      "Content-Disposition": `attachment; filename=ytlink.tk - ${title}.${file.ext}`,
    ...
    const resp = await drive.files.get(
      ...
      { responseType: "stream", headers: { range: { start, end } } }
    );
